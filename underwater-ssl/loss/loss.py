...
class Loss(object):
    ...
    def helmholtz_loss(self, pred, inputs, wave_speed, frequency):
        ...
        """
        laplacian = torch.autograd.grad(torch.autograd.grad(pred, inputs, grad_outputs=torch.ones_like(pred), create_graph=True)[0], inputs, grad_outputs=torch.ones_like(pred), create_graph=True)[0]
    ...
    def calculate_mse_loss_2(self, pred, target, eps=1e-8):
        ...
        if not isinstance(pred, torch.Tensor) or not isinstance(target, torch.Tensor):
            raise TypeError(f"Expected torch.Tensor, but got {type(pred)} and {type(target)}")
        ...
        si_sdr = 10 * torch.log10(torch.sum(target_scaled ** 2, dim=-1) / (torch.sum(noise ** 2, dim=-1) + eps))
    ...
    def plot_rays(self, rays, source, receiver):
        ...
        plt.scatter([receiver[1]], [receiver[0]], color='blue', label="Receiver", zorder=5)
